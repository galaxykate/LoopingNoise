...
var utilities = {
	...
	createProcessing: function(holderId, onUpdate, onDraw, onStart, dragFxns) {

		var canvas = document.createElement('canvas');
		canvas.style.cssText ="width:100%;height:100%"
		document.getElementById(holderId).appendChild(canvas);
	...
	words: {
		...
		syllables: {
			...
			last: "ant ent art ert e a i ie ei a a ae ea e e e y yay oy y a ia ea u y as en am us is art on in ath oll an o ang ing io i el ios ius ae ie ee i".split(" "),
...
function computeControlPoints(K) {
	// Create control points for each
	var cps = [], a = [], b = [], c = [], r = [];
	...
	cps[n - 1][0] = r[n-1]/b[n-1];
	for (i = n - 2; i >= 0; --i) {
		cps[i][0] = (r[i] - c[i] * cps[i+1][0]) / b[i];
	}

	/*we have p1, now compute p2*/
	for (i = 0; i < n-1; i++) {
		cps[i][1] = 2*K[i+1] - cps[i+1][0];
	}
	
	cps[n-1][1] = 0.5*(K[n] + cps[n-1][0]);
